...
public class SwiftFlutterBarcodeScannerPlugin: NSObject, FlutterPlugin, ScanBarcodeDelegate,FlutterStreamHandler {
  ...
  public func addNavigationBar(to viewController: UIViewController) {
                ...
                AVCaptureDevice.requestAccess(for: .video) { success in
                    DispatchQueue.main.async {
                        if success {
                            ...
                            let alert = UIAlertController(title: "米子市フレイル予防アプリ", message: "QRコードを読み取るためにカメラを使用します", preferredStyle: .alert)
                            
                            alert.addAction(UIAlertAction(title: "許可", style: .default, handler: { action in
